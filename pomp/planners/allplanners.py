...
def makePlanner(type,space,start,goal,
    ...
    #---------------------------------------------------------------------------------------------------------------
        ...
        planner = Policy_Guided_SST(controlSpace,objective,metric,checker,**params)
        #set direct steering functions for kinematic spaces 
        if isinstance(controlSpace,ControlSpaceAdaptor):
            planner.setControlSelector(KinematicControlSelector(controlSpace,controlSpace.nextStateSamplingRange))
    elif type == 'gdsst':
        planner = Gradient_Descent_SST(controlSpace,objective,metric,checker,**params)
        #set direct steering functions for kinematic spaces 
        if isinstance(controlSpace,ControlSpaceAdaptor):
            planner.setControlSelector(KinematicControlSelector(controlSpace,controlSpace.nextStateSamplingRange))
    elif type == 'pgdsst':
        planner = Policy_Guided_Gradient_Descent_SST(controlSpace,objective,metric,checker,**params)
        #set direct steering functions for kinematic spaces 
        if isinstance(controlSpace,ControlSpaceAdaptor):
            planner.setControlSelector(KinematicControlSelector(controlSpace,controlSpace.nextStateSamplingRange))
    elif type == 'rl-rrt':
