...
from HER.rl_modules.models import tdm_critic as critic
from HER.her_modules.her import tdm_her_sampler as her_sampler
...
class ddpg_agent:
    def __init__(self, args, env, env_params):
        ...
        self.sparse_reward_fn = lambda o, g, x=None: (np.mean(np.abs(o-g), axis=-1) < scale) -1
        # def smooth_reward(o, g, c=5):

        # self.vec_reward_func = lambda o, g, x=None: -np.abs(o-g) < env.env.distance_threshold
        self.her_module = her_sampler(self.args.replay_strategy, self.args.replay_k, 
            # reward_func=self.env.compute_reward, vec_reward_func=self.vec_reward_func)
            reward_func=self.sparse_reward_fn, vec_reward_func=self.vec_reward_func)
    ...
    def _update_network(self):
        ...
        vec_r_tensor = torch.tensor(transitions['vec_r'], dtype=torch.float32) 
        if self.args.cuda:
