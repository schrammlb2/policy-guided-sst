...
from HER.rl_modules.sac_models import value_prior_actor as actor
from HER.rl_modules.sac_models import dual_value_prior_critic
...
"""
# critic_constructor = value_prior_critic
critic_constructor = dual_value_prior_critic
...
class ddpg_agent:
            ...
            for _ in range(self.args.n_cycles):
                ...
                for _ in range(self.args.num_rollouts_per_mpi):
                    ...
                    for t in range(self.env_params['max_timesteps']):
                        with torch.no_grad():
                            input_tensor = self._preproc_inputs(obs, g, ag)
    ...
    def _preproc_inputs(self, obs, g, ag):
        obs_norm = self.o_norm.normalize(obs)
        g_norm = self.g_norm.normalize(g)
        ag_norm = self.g_norm.normalize(g)
        # concatenate the stuffs
        inputs = np.concatenate([obs_norm, g_norm, ag_norm])
    ...
    def _update_network(self):
        ...
        inputs_norm = np.concatenate([obs_norm, g_norm, ag_norm], axis=1)
        obs_next_norm = self.o_norm.normalize(transitions['obs_next'])
        g_next_norm = self.g_norm.normalize(transitions['g_next'])
        ag_next_norm = self.g_norm.normalize(transitions['ag_next'])
        inputs_next_norm = np.concatenate([obs_next_norm, g_next_norm, ag_next_norm], axis=1)
    ...
    def _eval_agent(self):
        ...
        for _ in range(self.args.n_test_rollouts):
            ...
            for _ in range(self.env_params['max_timesteps']):
                with torch.no_grad():
                    pi = self.actor_network.normed_forward(obs, g, ag, deterministic=True)
